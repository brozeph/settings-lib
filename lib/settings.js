...
module.exports = (function (settings) {
	 ...
	 * @param	parentPath	a namespaced field name used as a hash key
	 **/
	 ...
	 * @returns					a native Promise that resolves true or false
	 **/
	 ...
	 *
	 * @param 	objectList 	accepts array of objects
	 * @returns							a newly cloned object with the merged results
	 **/
	 ...
	 * @returns				a native Promise that resolves the file content
	 **/
	...
	loadBaseConfig = new Promise((resolve, reject) => {
	...
	loadCommandLineConfig = new Promise((resolve, reject) => {
		let
	...
	loadCommandLineSwitches = new Promise((resolve) => {
		let
			args = process.argv.slice(2), // removes node command
	...
	loadEnvironmentConfig = new Promise((resolve, reject) => {
		...
		settings.options.environmentSearchPaths.some((searchPath) => {
			let path = [
				searchPath,
			...
			return checkIfFileExists(path).then((exists) => {
				if (!exists) {
					return false;
				}
	...
	loadEnvironmentVariables = new Promise((resolve) => {
		let
			environmentKeys,
